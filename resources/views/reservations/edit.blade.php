        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 dark:bg-gray-800" x-data="{
                    selectedLocation: '{{ $reservation->location_id }}',
                    selectedPerson: '{{ $reservation->person_id }}',
                    people: @json($people),
                    services: @json($services),
                    selectedServices: @json($reservation->services),
                    ...
                    <form action="{{ route('reservations.update', $reservation->id) }}" method="POST" class="space-y-4">
                            ...
                            <select id="location_id" name="location_id"
                                ...
                                @foreach ($locations as $location)
                                    <option value="{{ $location->id }}"
                                        {{ $reservation->location_id == $location->id ? 'selected' : '' }}>
                                        {{ $location->name }}
                                    </option>
                                @endforeach
                            </select>
                                    ...
                                    <div class="flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-300">
                                        <label>
                                            <input type="checkbox" :value="service.id" name="services[]"
                                                class="form-checkbox" @if (in_array(service . id, selectedServices)) checked @endif
                        ...
                        <button type="submit" class="px-4 py-2 text-white bg-blue-500 rounded">Update
                            Reservation</button>
                    </form>
                </div>
            </div>
